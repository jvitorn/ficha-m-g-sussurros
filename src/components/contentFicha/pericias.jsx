...
export default function ContentFichaPericias() {
  ...
  useEffect(() => {
    const novasPericias = pericias.map((pericia) => {
      if (pericia.ativo) {
        const atributoCorrespondente = atributos.find(
          (attr) => attr.sigla === pericia.atributo
        );
        if (atributoCorrespondente) {
          // Calcula o bônus como metade do valor do atributo (arredondado para baixo)
          return {
            ...pericia,
            valor: Math.floor(atributoCorrespondente.valor / 2),
          };
        }
      }
      return pericia;
    ...
    if (novoEstado) {
      ...
      if (atributoCorrespondente) {
        handleChangePericia(
          index,
          "valor",
          Number(atributoCorrespondente.valor)
        );
      }
    } else {
        ...
        <Table striped bordered hover responsive>
          ...
          <tbody>
            {pericias.map((per, index) => (
              <tr key={index}>
                {/* Células ocultas dentro de td */}
                <td style={{ display: "none" }}>
                  <input
                    type="hidden"
                    name={`pericias[${index}].nome`}
                    value={per.nome}
                  />
                </td>
                <td style={{ display: "none" }}>
                ...
                <td style={{ display: "none" }}>
                  <input
                    type="hidden"
                    name={`pericias[${index}].ativo`}
                    value={per.ativo ? "true" : "false"}
                  />
                </td>
